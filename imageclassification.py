...
print('\nCoarse class: {}'.format(np.unique(coarse_TrLabels)))
...
print('\nFine class: {}'.format(np.unique(fine_TrLabels)))
...
print('\nTotal images from TRAINING DATASET: {}'.format(len(idx)))
idx = np.array(idx)
...
print('FINE CLASS (training images): {}\n'.format(uniq_fineClass))
...
print('\nTotal images from TESTING DATASET: {}'.format(len(idx)))
idx = np.array(idx)
...
print('FINE CLASS (testing images): {}\n'.format(uniq_fineClass))
...
data_augmentation = tf.keras.Sequential([
    # reduce augmentation strength to prevent breaking the image 
    # tf.keras.layers.RandomFlip("horizontal"),
    # tf.keras.layers.RandomRotation(0.05),  # smaller rotation
    # tf.keras.layers.RandomZoom(0.1),       # smaller zoom
    # tf.keras.layers.RandomContrast(0.05),  # smaller contrast change
...
print("\nNew labels:", np.unique(train_labels))
...
for class_id in range(len(str_class)):
    # idx = np.where(test_labels == class_id)[0][0]  # pick the [0] [n] occurrence of each class
    # selected_indices.append(idx)
    idxs = np.where(test_labels == class_id)[0][:2]  # get first 2 images
    selected_indices.extend(idxs)

print("\nSelected test indices: ", selected_indices)
print("\n\n\n")
...
k = 1
while os.path.exists(f"val-loss/val-loss_plot_{k}.png"):
    k += 1

plt.savefig(f"val-loss/val-loss_plot_{k}.png", dpi=300)
plt.show()

# TEST THE MODEL
print(test_images.shape)
print("\nClass in the testing image: {}".format(np.unique(test_labels)))
...
plt.title("Confusion Matrix (Test Accuracy: {:.2f}%)".format(test_acc * 100))
